...
def main():
    ...
    parser.add_argument('--init_type', type=str, default='default',
                        choices=['default', 'low_rank', 'sparse', 'small'],
                        help='Initialization type: default, low_rank, sparse, or small (default: default)')
    parser.add_argument('--rank_ratio', type=float, default=0.5,
                        help='Rank ratio for low_rank init (default: 0.5)')
    parser.add_argument('--sparsity', type=float, default=0.9,
                        help='Sparsity for sparse init (0.9 = 90%% zeros) (default: 0.9)')
    parser.add_argument('--init_scale', type=float, default=0.01,
                        help='Scale for small init (default: 0.01)')
    ...
    # Create data loaders
    batch_size = min(args.batch_size, len(train_data) // 2)
    
    if args.multi_task:
        # Use balanced batch sampling for multi-task
        from balanced_batch_sampling import create_balanced_dataloaders
        train_loader, val_loader = create_balanced_dataloaders(
            train_data, val_data, vocab, task_mix, batch_size, multi_task=True
        )
    else:
        # Standard dataloaders for single-task
        X_train, y_train = prepare_data(train_data, vocab, multi_task=False)
        X_val, y_val = prepare_data(val_data, vocab, multi_task=False)
    ...
    if args.init_type != 'default':
        print(f"\nApplying {args.init_type} initialization...")
        from initializations import apply_initialization, analyze_initialization
        
        if args.init_type == 'low_rank':
            apply_initialization(model, init_type='low_rank', rank_ratio=args.rank_ratio)
        elif args.init_type == 'sparse':
            apply_initialization(model, init_type='sparse', sparsity=args.sparsity)
        elif args.init_type == 'small':
            apply_initialization(model, init_type='small', scale=args.init_scale)
        
        # Analyze initialization
        analyze_initialization(model, verbose=True)
    ...
    if args.init_type == 'low_rank':
        config['rank_ratio'] = args.rank_ratio
    elif args.init_type == 'sparse':
