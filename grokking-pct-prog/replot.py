...
def replot_from_checkpoint(checkpoint_dir, save_path, num_steps_override=None):
    ...
    # Recover grokking percentages
    grok_steps = history.get('grok_steps', {})
    grok_pcts = history.get('grok_pcts', {})
    # If grok_pcts weren't stored, compute them now
    ...
    ax2.set_xlim(left=max(x_vals[0], 0.001))
    ax2.set_xscale('log')
    ax2.set_yscale('log')
